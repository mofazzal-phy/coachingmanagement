<template>
  <div class="page-container">
    ...
    <div v-else-if="error" class="error-state">
      <p>⚠️ {{ error }}</p>
      <button class="btn btn-outline" @click="fetchRoutine">Try Again</button>
    </div>
...
const totalSlots = computed(() => routine.value.length)
...
async function fetchRoutine() {
  ...
  error.value = null
  try {
    const res = await classRoutineService.getStudentRoutine()
    // API returns { data: { weekly: {...}, flat: [...] } }
    const data = res.data?.data || res.data || {}
    routine.value = data.flat || []
  } catch (e) {
    error.value = e.response?.data?.message || 'Failed to load routine'
  } finally {
    loading.value = false
  }
}

<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Fee Types</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Fee Type</button>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading fee types...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">💰</div><h3>No Fee Types Found</h3><p>Create your first fee type</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Fee Type</button>
    </div>
    <div v-else class="table-container">
      <table class="data-table">
        <thead>
          <tr><th>Name</th><th>Code</th><th>Amount</th><th>Category</th><th>Description</th><th>Actions</th></tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td>
              <span class="cat-badge" :class="'cat-' + (item.category || 'monthly')">
                {{ item.category_label || item.category || 'Monthly' }}
              </span>
            </td>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Fee Type' : 'Create Fee Type' }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input v-model="form.name" class="form-control" placeholder="e.g., Tuition Fee" />
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Code</label>
              <input v-model="form.code" class="form-control" placeholder="e.g., TUIT" />
            </div>
            <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.name || !form.amount || dialogLoading">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete fee type <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const form = ref({ name: '', code: '', amount: 0, category: 'monthly', description: '' })
...
const loadItems = async () => {
  ...
  try { const res = await financeService.feeTypes.list(); items.value = res.data.data || [] }
  catch (err) { error.value = err.response?.data?.message || 'Failed to load' }
  finally { loading.value = false }
}

const formatCurrency = (amount) => {
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'BDT', minimumFractionDigits: 0 }).format(amount || 0)
}

const openCreateDialog = () => { editingItem.value = null; form.value = { name: '', code: '', amount: 0, category: 'monthly', description: '' }; dialogError.value = null; showDialog.value = true }
const openEditDialog = (item) => { editingItem.value = item; form.value = { name: item.name, code: item.code || '', amount: item.amount || 0, category: item.category || 'monthly', description: item.description || '' }; dialogError.value = null; showDialog.value = true }
const closeDialog = () => { showDialog.value = false; editingItem.value = null }

const saveItem = async () => {
  dialogLoading.value = true; dialogError.value = null
  try {
    if (editingItem.value) await financeService.feeTypes.update(editingItem.value.id, form.value)
    else await financeService.feeTypes.create(form.value)
...
const deleteItem = async () => {
  ...
  try { await financeService.feeTypes.delete(selectedItem.value.id); showDeleteDialog.value = false; loadItems() }

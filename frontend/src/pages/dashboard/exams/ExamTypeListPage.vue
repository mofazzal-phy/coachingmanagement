<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Exam Types</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Exam Type</button>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading exam types...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">📝</div><h3>No Exam Types Found</h3><p>Create your first exam type</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Exam Type</button>
    </div>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Exam Type' : 'Create Exam Type' }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input v-model="form.name" class="form-control" placeholder="e.g., Midterm" />
          </div>
          <div class="form-group">
            <label>Code</label>
            <input v-model="form.code" class="form-control" placeholder="e.g., MID" />
          </div>
          <div class="form-group">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete exam type <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const loadItems = async () => {
  ...
  try { const res = await examService.types.list(); items.value = res.data.data || [] }
  catch (err) { error.value = err.response?.data?.message || 'Failed to load' }
  finally { loading.value = false }
}

const openCreateDialog = () => { editingItem.value = null; form.value = { name: '', code: '', description: '' }; dialogError.value = null; showDialog.value = true }
const openEditDialog = (item) => { editingItem.value = item; form.value = { name: item.name, code: item.code || '', description: item.description || '' }; dialogError.value = null; showDialog.value = true }
const closeDialog = () => { showDialog.value = false; editingItem.value = null }

const saveItem = async () => {
  dialogLoading.value = true; dialogError.value = null
  try {
    if (editingItem.value) await examService.types.update(editingItem.value.id, form.value)
    else await examService.types.create(form.value)
...
const deleteItem = async () => {
  ...
  try { await examService.types.delete(selectedItem.value.id); showDeleteDialog.value = false; loadItems() }

<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Designations</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Designation</button>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading designations...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">📋</div><h3>No Designations Found</h3><p>Create your first designation</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Designation</button>
    </div>
    <div v-else class="table-container">
      <table class="data-table">
        <thead>
          <tr><th>Name</th><th>Department</th><th>Description</th><th>Actions</th></tr>
        </thead>
        <tbody>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Designation' : 'Add Designation' }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input v-model="form.name" class="form-control" placeholder="e.g., Senior Teacher" />
          </div>
          <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.name || !form.department_id || dialogLoading">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete designation <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const form = ref({ name: '', department_id: '', description: '' })
...
const loadItems = async () => {
  ...
  try { const res = await hrService.designations.list(); items.value = res.data.data || [] }
  catch (err) { error.value = err.response?.data?.message || 'Failed to load' }
  finally { loading.value = false }
}

const loadDepartments = async () => {
  try { const res = await hrService.departments.list(); departments.value = res.data.data || [] }
  catch {}
}

const openCreateDialog = () => { editingItem.value = null; form.value = { name: '', department_id: '', description: '' }; dialogError.value = null; showDialog.value = true }
const openEditDialog = (item) => { editingItem.value = item; form.value = { name: item.name, department_id: item.department_id || '', description: item.description || '' }; dialogError.value = null; showDialog.value = true }
const closeDialog = () => { showDialog.value = false; editingItem.value = null }

const saveItem = async () => {
  dialogLoading.value = true; dialogError.value = null
  try {
    if (editingItem.value) await hrService.designations.update(editingItem.value.id, form.value)
    else await hrService.designations.create(form.value)
...
const deleteItem = async () => {
  ...
  try { await hrService.designations.delete(selectedItem.value.id); showDeleteDialog.value = false; loadItems() }
  catch (err) { error.value = err.response?.data?.message || 'Failed to delete'; showDeleteDialog.value = false }
  finally { deleteLoading.value = false }
}

onMounted(() => { loadItems(); loadDepartments() })
</script>

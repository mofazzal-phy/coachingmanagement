<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Departments</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Department</button>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading departments...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">🏢</div><h3>No Departments Found</h3><p>Create your first department</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Department</button>
    </div>
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input v-model="form.name" class="form-control" placeholder="e.g., Science" />
          </div>
          <div class="form-group">
            <label>Code</label>
            <input v-model="form.code" class="form-control" placeholder="e.g., SCI" />
          </div>
          <div class="form-group">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete department <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const loadItems = async () => {
  ...
  try { const res = await hrService.departments.list(); items.value = res.data.data || [] }
...
const saveItem = async () => {
  ...
  try {
    if (editingItem.value) await hrService.departments.update(editingItem.value.id, form.value)
    else await hrService.departments.create(form.value)
...
const deleteItem = async () => {
  ...
  try { await hrService.departments.delete(selectedItem.value.id); showDeleteDialog.value = false; loadItems() }

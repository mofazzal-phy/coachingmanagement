<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Employees</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Employee</button>
      </div>
    </div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">👥</div><h3>No Employees Found</h3><p>Add your first employee</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Employee</button>
    </div>
    <div v-else class="table-container">
      <table class="data-table">
        <thead>
          <tr><th>Name</th><th>Employee ID</th><th>Department</th><th>Designation</th><th>Phone</th><th>Status</th><th>Actions</th></tr>
        </thead>
        <tbody>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog modal-lg">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Employee' : 'Add Employee' }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-row">
            ...
            <div class="form-group">
              ...
              <input v-model="form.employee_id" class="form-control" placeholder="EMP-001" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.name || !form.department_id || !form.designation_id || dialogLoading">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete employee <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const form = ref({ name: '', employee_id: '', department_id: '', designation_id: '', email: '', phone: '', status: 'active', joining_date: '', address: '' })
...
const loadItems = async () => {
  ...
  try { const res = await hrService.employees.list(); items.value = res.data.data || [] }
...
const loadDependencies = async () => {
  try {
    const [deptRes, desigRes] = await Promise.all([hrService.departments.list(), hrService.designations.list()])
    departments.value = deptRes.data.data || []
    designations.value = desigRes.data.data || []
  } catch {}
}

const openCreateDialog = () => { editingItem.value = null; form.value = { name: '', employee_id: '', department_id: '', designation_id: '', email: '', phone: '', status: 'active', joining_date: '', address: '' }; dialogError.value = null; showDialog.value = true }
const openEditDialog = (item) => { editingItem.value = item; form.value = { name: item.name, employee_id: item.employee_id || '', department_id: item.department_id || '', designation_id: item.designation_id || '', email: item.email || '', phone: item.phone || '', status: item.status || 'active', joining_date: item.joining_date?.split('T')[0] || '', address: item.address || '' }; dialogError.value = null; showDialog.value = true }
const closeDialog = () => { showDialog.value = false; editingItem.value = null }

const saveItem = async () => {
  dialogLoading.value = true; dialogError.value = null
  try {
    if (editingItem.value) await hrService.employees.update(editingItem.value.id, form.value)
    else await hrService.employees.create(form.value)
...
const deleteItem = async () => {
  ...
  try { await hrService.employees.delete(selectedItem.value.id); showDeleteDialog.value = false; loadItems() }

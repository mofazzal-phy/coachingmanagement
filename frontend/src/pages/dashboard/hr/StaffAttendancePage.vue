<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Staff Attendance</h1><span class="badge-count">{{ items.length }} records</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openMarkDialog">+ Mark Attendance</button>
      </div>
    </div>
    ...
    <div class="filters">
      <div class="form-group">
        <label>Date</label>
        <input v-model="filters.date" type="date" class="form-control" @change="loadItems" />
      </div>
      <div class="form-group">
        <label>Department</label>
        <select v-model="filters.department_id" class="form-control" @change="loadItems">
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">📋</div><h3>No Attendance Records</h3><p>Mark attendance for today</p>
      <button class="btn btn-primary" @click="openMarkDialog">+ Mark Attendance</button>
    </div>
    <div v-else class="table-container">
      <table class="data-table">
        <thead>
          <tr><th>Employee</th><th>Department</th><th>Date</th><th>Status</th><th>In Time</th><th>Out Time</th><th>Actions</th></tr>
        </thead>
        <tbody>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Attendance' : 'Mark Attendance' }}</h3>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.employee_id || !form.date || !form.status || dialogLoading">
            {{ dialogLoading ? 'Saving...' : (editingItem ? 'Update' : 'Mark') }}
          </button>
        </div>
      </div>
    </div>
...
const filters = ref({ date: new Date().toISOString().split('T')[0], department_id: '', status: '' })
const form = ref({ employee_id: '', date: new Date().toISOString().split('T')[0], status: 'present', in_time: '', out_time: '', remarks: '' })

const loadItems = async () => {
  loading.value = true; error.value = null
  try { const res = await hrService.staffAttendance.list(filters.value); items.value = res.data.data || [] }
...
const loadDependencies = async () => {
  try {
    const [empRes, deptRes] = await Promise.all([hrService.employees.list(), hrService.departments.list()])
...
const openMarkDialog = () => { editingItem.value = null; form.value = { employee_id: '', date: new Date().toISOString().split('T')[0], status: 'present', in_time: '', out_time: '', remarks: '' }; dialogError.value = null; showDialog.value = true }
const openEditDialog = (item) => { editingItem.value = item; form.value = { employee_id: item.employee_id || '', date: item.date?.split('T')[0] || '', status: item.status || 'present', in_time: item.in_time || '', out_time: item.out_time || '', remarks: item.remarks || '' }; dialogError.value = null; showDialog.value = true }
const closeDialog = () => { showDialog.value = false; editingItem.value = null }

const saveItem = async () => {
  dialogLoading.value = true; dialogError.value = null
  try {
    if (editingItem.value) await hrService.staffAttendance.update(editingItem.value.id, form.value)
    else await hrService.staffAttendance.create(form.value)
...
.filters { display: flex; gap: 1rem; margin-bottom: 1.25rem; flex-wrap: wrap; background: var(--bg-card); padding: 1rem; border-radius: 12px; box-shadow: var(--shadow-sm); }
.filters .form-group { margin-bottom: 0; min-width: 180px; }
.filters .form-group label { font-size: 0.75rem; margin-bottom: 0.25rem; }
...
.status-badge.half-day { background: #dbeafe; color: #2563eb; }
.status-badge.holiday { background: #f3f4f6; color: var(--text-muted); }

<template>
  <div class="page-container">
    <div class="page-header">
      <div class="header-left"><h1>Subjects</h1><span class="badge-count">{{ items.length }} total</span></div>
      <div class="header-actions">
        <button class="btn btn-outline" @click="loadItems" :disabled="loading">🔄 Refresh</button>
        <button class="btn btn-primary" @click="openCreateDialog">+ Add Subject</button>
      </div>
    </div>

    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading subjects...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      <div class="empty-icon">📚</div><h3>No Subjects Found</h3><p>Create your first subject</p>
      <button class="btn btn-primary" @click="openCreateDialog">+ Add Subject</button>
    </div>
    <div v-else class="table-container">
      <table class="data-table">
        <thead>
          <tr><th>Name</th><th>Code</th><th>Type</th><th>Groups</th><th>Credit Hours</th><th>Actions</th></tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td>{{ item.code || '—' }}</td>
            <td>
              <span class="type-badge" :class="item.type?.toLowerCase()">{{ item.type || 'Core' }}</span>
            </td>
            <td>
              <span v-if="item.groups && item.groups.length" class="group-tags">
                <span v-for="g in item.groups" :key="g.id" class="group-tag">{{ g.name }}</span>
              </span>
              <span v-else class="text-muted">—</span>
            </td>
            <td>{{ item.credit_hours || 0 }}</td>
            <td>
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit Subject' : 'Create Subject' }}</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input v-model="form.name" class="form-control" placeholder="e.g., Mathematics" />
          </div>
          <div class="form-group">
            <label>Code <span class="required">*</span></label>
            <input v-model="form.code" class="form-control" placeholder="e.g., MATH" />
          </div>
          <div class="form-group">
            <label>Type</label>
          ...
          <div class="form-group">
            ...
            <select v-model="form.group_ids" class="form-control" multiple>
              <option v-for="g in allGroups" :key="g.id" :value="g.id">{{ g.name }}</option>
            </select>
            <small class="form-help">Hold Ctrl/Cmd to select multiple groups. Only needed for classes 9-12.</small>
          </div>
          <div class="form-group">
            <label>Credit Hours</label>
            <input v-model="form.credit_hours" type="number" min="0" max="20" class="form-control" placeholder="e.g., 3" />
          </div>
          <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.name || !form.code || dialogLoading">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete subject <strong>{{ selectedItem?.name }}</strong>?</p><p class="text-danger">This cannot be undone.</p></div>
        <div class="modal-footer">
...
const form = ref({ name: '', code: '', type: 'core', group_ids: [], credit_hours: '', description: '' })
...
const loadItems = async () => {
  ...
  try {
    const res = await academicService.subjects.list()
...
const loadGroups = async () => {
  try {
    const res = await academicService.groups.listAll()
...
const openCreateDialog = () => {
  ...
  form.value = { name: '', code: '', type: 'core', group_ids: [], credit_hours: '', description: '' }
...
const saveItem = async () => {
  ...
  try {
    ...
    if (editingItem.value) await academicService.subjects.update(editingItem.value.id, payload)
    else await academicService.subjects.create(payload)
...
const deleteItem = async () => {
  ...
  try {
    await academicService.subjects.delete(selectedItem.value.id)
...
.type-badge { padding: 0.2rem 0.6rem; border-radius: 6px; font-size: 0.75rem; font-weight: 600; background: #f3f4f6; color: var(--text-secondary); }
.type-badge.core { background: #e8eaf6; color: #4f46e5; }
.type-badge.elective { background: #e3f2fd; color: #2563eb; }
.type-badge.optional { background: #fef3c7; color: #d97706; }
.group-tags { display: flex; gap: 4px; flex-wrap: wrap; }
.group-tag { background: #fef3c7; color: #92400e; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: 500; }
...
.form-control[multiple] { min-height: 80px; }
.form-help { display: block; font-size: 0.75rem; color: var(--text-muted); margin-top: 4px; }

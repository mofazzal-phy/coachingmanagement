<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <div class="header-actions">
        ...
        <button class="btn btn-primary" @click="openCreateDialog">
          <span class="btn-text-visible">+ New Success Story</span>
          <span class="btn-icon-hidden">+</span>
        </button>
      </div>
    </div>
    ...
    <div class="filter-bar">
      <input v-model="filters.search" class="form-control filter-search" placeholder="Search title, student, exam..." @keyup.enter="loadItems" />
    ...
    <div v-if="loading" class="loading-state"><div class="spinner"></div><p>Loading success stories...</p></div>
    ...
    <div v-else-if="items.length === 0" class="empty-state">
      ...
      <button class="btn btn-primary" @click="openCreateDialog">+ New Success Story</button>
    </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="item in items" :key="item.id">
                <td>
                  <div class="name-cell">
                    <img v-if="item.featured_image_url" :src="item.featured_image_url" class="avatar-thumb" alt="" />
                    <div>
                      <strong>{{ item.title }}</strong>
                      <span v-if="item.excerpt" class="sub-text">{{ truncate(item.excerpt, 60) }}</span>
                ...
                <td>
                  ...
                  <span v-if="item.achievement_year" class="sub-text">{{ item.achievement_year }}</span>
                </td>
                ...
                <td class="actions-cell">
                  <CmsActionDropdown
                    ...
                    edit-label="Edit Story"
                    delete-label="Delete Story"
                    status-mode="active"
            ...
            <div class="card-title-section">
              <div class="name-cell">
                ...
                <div>
                  ...
                  <span v-if="item.student_name" class="sub-text">{{ item.student_name }}</span>
                </div>
              </div>
          ...
          <div class="card-body">
            <div class="card-info-grid">
              <div class="info-item">
                ...
                <span class="info-value">{{ item.exam_name || '—' }}</span>
              </div>
              <div class="info-item">
                <span class="info-label">Year</span>
                <span class="info-value">{{ item.achievement_year || '—' }}</span>
              </div>
              <div class="info-item">
    ...
    <div class="modal-overlay" v-if="showDialog" @click.self="closeDialog">
      <div class="modal-dialog modal-lg">
        <div class="modal-header">
          <h3>{{ editingItem ? 'Edit' : 'Create' }} Success Story</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <textarea v-model="form.story" class="form-control" rows="6"></textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Student Name</label>
              <input v-model="form.student_name" class="form-control" />
            </div>
            <div class="form-group">
              <label>Exam Name</label>
              <input v-model="form.exam_name" class="form-control" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Achievement Year</label>
              <input v-model="form.achievement_year" class="form-control" placeholder="e.g. 2025" />
            </div>
            <div class="form-group">
              <label>Result Summary</label>
              <input v-model="form.result_summary" class="form-control" placeholder="e.g. AIR 42" />
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveItem" :disabled="!form.title || !form.slug || !form.story || dialogLoading">
    ...
    <div class="modal-overlay" v-if="showDeleteDialog" @click.self="showDeleteDialog = false">
      <div class="modal-dialog modal-sm">
        ...
        <div class="modal-body"><p>Delete success story <strong>{{ selectedItem?.title }}</strong>?</p></div>
        <div class="modal-footer">
...
const {
  ...
  closeReject,
  confirmReject,
} = useCmsApproval(cmsService.successStories)
...
const loadItems = async () => {
  ...
  try {
    ...
    const res = await cmsService.successStories.list(params)
    items.value = cmsService.extractList(res)
  } catch (err) {
    error.value = err.response?.data?.message || 'Failed to load success stories'
  } finally {
    loading.value = false
  }
}
...
const onImageSelect = async (event) => {
  ...
  if (!file) return
  try {
    const res = await cmsService.media.upload(file, 'image', 'success-stories')
    const data = res.data?.data || res.data
    form.value.featured_image = data.path
    form.value.featured_image_url = data.url
  } catch (err) {
...
const saveItem = async () => {
  ...
  try {
    if (editingItem.value) {
      await cmsService.successStories.update(editingItem.value.id, payload)
    } else {
      await cmsService.successStories.create(payload)
...
const deleteItem = async () => {
  ...
  try {
    await cmsService.successStories.delete(selectedItem.value.id)
...
.avatar-thumb { width: 40px; height: 40px; border-radius: 8px; object-fit: cover; flex-shrink: 0; }
...
.image-preview img { max-width: 120px; max-height: 120px; margin-top: 0.5rem; border-radius: 8px; object-fit: cover; }
